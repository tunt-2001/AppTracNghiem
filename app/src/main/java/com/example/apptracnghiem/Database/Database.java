...
public class Database extends SQLiteOpenHelper {
    ...
    public void onCreate(SQLiteDatabase sqLiteDatabase) {
        ...
        final String CATEGORIES_TABLE = "CREATE TABLE " +
                ...
                Table.CategoryTable._ID + " INTEGER PRIMARY KEY AUTOINCREMENT, " +
                Table.CategoryTable.COLUMN_NAME + " TEXT " +
                ")";
        //biến bảng question
//        db.execSQL("DELETE FROM " + Table.CategoryTable.TABLE_NAME);
        ...
        final String QUESTIONS_TABLE = "CREATE TABLE " +
                ...
                Table.CategoryTable.TABLE_NAME + "(" + Table.CategoryTable._ID + ")" + "ON DELETE CASCADE" +
                ")";


        String LOGIN_TABLE = "CREATE TABLE " +
                Table.LoginTable.TABLE_NAME + " ( " +
                Table.LoginTable.COLUMN_USER + " TEXT PRIMARY KEY, " +
                Table.LoginTable.COLUMN_PASS + " TEXT, " +
                "'')";

        String INFOMATION_TABLE = "create table if not exists ThongTin(hoTen text primary key, email text, NgaySinh text, sdt text)";
        //tạo bảng
...
//        CreateQuestions();
    ...
    public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
        ...
        db.execSQL("DROP TABLE IF EXISTS " + Table.CategoryTable.COLUMN_NAME);
    ...
    private void insertCategories(Category category){
        ...
        values.put(Table.CategoryTable.COLUMN_NAME, category.getName());
        db.insert(Table.CategoryTable.TABLE_NAME, null,values);
    ...
    private void CreateCategory(){
        Category c1 = new Category("Toán");
        //môn văn id ==1
        insertCategories(c1);
        //môn sử id ==2
        Category c2 = new Category("Tiếng Việt");
        insertCategories(c2);
        //môn địa id ==3
        Category c3 = new Category("Tiếng Anh");
        insertCategories(c3);
        Category c4 = new Category("Khoa Học");
        insertCategories(c4);
        Category c5 = new Category("Đố Vui");
        insertCategories(c5);
        Category c6 = new Category("Lịch Sử");
        insertCategories(c6);
        Category c7 = new Category("Địa Lý");
        insertCategories(c7);
        Category c8 = new Category("Vật Lý");
        insertCategories(c8);
    }
    //insert câu hỏi và đáp án
...
//    @SuppressLint("Range")
    public List<Category> getDataCategories(){
        ...
        Cursor c = db.rawQuery("SELECT * FROM " + Table.CategoryTable.TABLE_NAME,null);

        if (c.moveToFirst()){
            do{
                Category category = new Category();
                category.setId(c.getInt(c.getColumnIndexOrThrow(Table.CategoryTable._ID)));
                category.setName(c.getString(c.getColumnIndexOrThrow(Table.CategoryTable.COLUMN_NAME)));
    ...
    //lấy dữ liệu câu hỏi và đáp án có id = id category theo chủ đề
//    @SuppressLint("Range")
//    @SuppressLint("Range")
